...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  console.log('Simple Generate QR API called:', req.method, req.body);
  ...
  try {
    ...
    console.log('Simple QR generation successful - no Firebase operations');
    res.status(200).json({ 
      token,
      message: 'QR code generated successfully (simple version without Firebase)',
      seatId,
      seatNumber,
      floorId,
      userId,
      expiresAt: new Date(expiresAt).toISOString(),
      oneTimeToken: oneTimeToken.substring(0, 8) + '...' // 일부만 표시
    });
  } catch (error) {
